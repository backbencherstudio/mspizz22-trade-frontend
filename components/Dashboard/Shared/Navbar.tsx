...
import { Search, Mail, User, HelpCircle, Menu, X, LogOutIcon } from "lucide-react";
...
export default function Navbar() {
  ...
  const navItems = [
    {
      label: "Accounts", link: "/accounts", subItems: [
        "Summary",
        "Balances",
        "Positions",
        "Realized Gain / Loss",
    ...
    { label: "Research", link: "/research", subItems: ["r", "t", "Positionsbb", "c"] },
    { label: "Move Money", link: "/move-money", subItems: ["vv", "Baldfdances", "ddd", "ff"] },
    { label: "Products", link: "/products", subItems: ["tt", "yy", "uu", "Realiiized"] },
    { label: "Learn", link: "/learn", subItems: [] },
    { label: "How To", link: "/how-to", subItems: [] }
  ];

  const rightSideActions = [
    { label: "Messages", icon: <MessageIcon className="lg:size-4  size-5 2xl:size-6" />, href: "#" },
    { label: "Support", icon: <SupportIcon className="lg:size-4 size-5 2xl:size-6" />, href: "#" },
    { label: "Profile", icon: <ProfileIcon className="lg:size-4 size-5.5 2xl:size-6" />, href: "#" },
  ];
            ...
            {navItems.map((item, index) => {
              return (
                <button
                  ...
                  className={`cursor-pointer h-full text-[#596772] hover:text-[#006E99] 2xl:text-lg lg:text-sm text-base font-medium tracking-[-.09px] transition-colors duration-200 ${activeTab === item.link
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-white border-t border-gray-200">
            ...
            {navItems.map((item, index) => {
              return (
                <div key={index}>
                  <button
                    onClick={() => handleTabClick(item.link)}
                    className={`block w-full text-left px-3 py-2 text-base font-medium transition-colors duration-200 ${activeTab === item.link
          ...
          <div className="border-t border-gray-200 px-4 py-3 space-y-3">
            {/* Mobile Search */}
            <div className='relative'>
              <input className="inputStyle w-full h-[36px] rounded-md transition-colors pl-[38px]" placeholder="Search" />
              <SearchIcon className="absolute top-1/2 w-[18px] h-[18px] left-[8px] transform -translate-y-1/2 cursor-pointer" />
            </div>
            {rightSideActions.map((action, index) => (
              <Link href={action?.href} key={index} className={`flex gap-2 px-3 items-center text-[#596772]  leading-[-0.18px]`}>
                <span>{action.icon}</span>
            ...
            <button className="flex gap-2 w-full text-left px-3 text-base font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-50">
              <span><LogOutIcon className="w-5.5" /></span> Log Out
            </button>
          </div>
        </div>
      )}
